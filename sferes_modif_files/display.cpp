...
namespace fastsim
{
  ...
  void Display :: _blit_map()
  {
	...
	} else if (_map->get_pixel(i, j) == Map::black) {
	  _put_pixel(_map_bmp, i, j, 1, 1, 1); // 254 not to be regarded as obstacle
	} else if (_map->get_pixel(i, j) == Map::free) {
	  _put_pixel(_map_bmp, i, j, 128, 128, 128);
	} else if (_map->get_pixel(i, j) == Map::white) {
	  _put_pixel(_map_bmp, i, j, 254, 254, 254); // 254 not to be regarded as free
	}
    ...
    _map(m), _robot(r)
  {
    ...
    // List of flags values: http://sdl.beuc.net/sdl.wiki/SDL_SetVideoMode
    _screen = SDL_SetVideoMode(_w, _h,
    	       32, SDL_SWSURFACE);
    SDL_WM_SetCaption( "mRF robot simulation", 0 );
